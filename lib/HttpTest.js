    ...
    _.forEach(self.tests, function(test) {
      if (!(test instanceof Test)) {
        ...
        var _test = {
          ...
          doTest: function() {
            try {
              var response = self.sync._executeHttpTest(test, history)
            } finally {
              history._addReq(test.name, response ? response.req : undefined)
              history._addRes(test.name, response)
            }
          }
        }
        if (test.setup) {
