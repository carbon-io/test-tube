...
var Test = oo({
   ...
   * @member context
   *
   * A mostly free-form bag of properties that will be passed down through 
   * the test tree. If you need to share data between tests, use this object.
   */
  context: {
    $property: {
      get: function() {
  ...
  _initContext: function() {
    ...
    this.context = new this.testContextClass()
  },
  ...
  run: function(done) {
    ...
    try {
      try {
        ...
        if (this.tests) {
          ...
          this.tests.forEach(function(test) {
            ...
            // pass the context
            test.context = self.context
            childResult = test.sync.run()
      ...
      if (result.passed && self.errorExpected && !result.error) {
        result.passed = false
